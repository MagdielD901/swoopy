...
import {
  ...
  SafeAreaView, StatusBar, Dimensions, Animated, PanResponder
} from 'react-native';
...
// --- COMPONENTE DE PARTÍCULA DE EXPLOSIÓN ---
...
export default function Orbix3D() {
  ...
  const glow = useRef(new Animated.Value(0)).current; // Nuevo: Controla la intensidad de luz
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.timing(spin, {
        toValue: 1,
        duration: emotional === 'ACTIVA' ? 10000 : 20000,
        useNativeDriver: true,
      })
    ).start();

    // Loop de Pulso y Glow combinado
    Animated.loop(
      Animated.parallel([
        Animated.sequence([
          Animated.timing(pulse, {
            toValue: emotional === 'INESTABLE' ? 1.12 : 1.05,
            duration: emotional === 'ACTIVA' ? 600 : 1500,
            useNativeDriver: true,
          }),
          Animated.timing(pulse, {
            toValue: 1,
        ...
        Animated.sequence([
          Animated.timing(glow, {
            toValue: 1,
            duration: emotional === 'INESTABLE' ? 400 : 1500,
            useNativeDriver: false, // Shadow props no soportan native driver
          }),
          Animated.timing(glow, {
            toValue: 0,
            duration: emotional === 'INESTABLE' ? 400 : 1500,
  ...
  const triggerHaptic = (currentMode) => {
    switch (currentMode) {
      case 'OVERLOAD':
  ...
  // Interpolación para el Glow dinámico
  const shadowSpread = glow.interpolate({
    inputRange: [0, 1],
    outputRange: [20, emotional === 'INESTABLE' ? 80 : 45]
  });
        ...
        <ScrollView contentContainerStyle={{ alignItems: 'center', paddingBottom: 60 }}>
          <View style={styles.orbScene}>
            ...
            <Animated.View
              ...
              style={[
                styles.orbWrapper,
                { opacity: orbOpacity, transform: [{ rotateX: rX }, { rotateY: rY }, { scale: pulse }] }
              ]}
            >
              {/* Capa de Glow Dinámico detras del núcleo */}
              <Animated.View style={[
                styles.dynamicGlow, 
                { 
              ...
              <Animated.View style={[styles.explosion, { 
                borderColor: MODES[mode].color, 
                opacity: explodeO, 
                transform: [{ scale: explodeS }] 
              }]} />
              ...
              <View style={[styles.orbCore, { 
                ...
                shadowColor: MODES[mode].color, 
                borderColor: MODES[mode].color + '88' 
              }]}>
            ...
            <Text style={styles.hintText}>
              {isExploded ? "REINICIANDO NÚCLEO..." : "Presiona el núcleo para sobrecargar el sistema"}
            </Text>
          </View>

          <View style={styles.groupsSection}>
            <Text style={[styles.sectionLabel, { color: MODES[mode].color }]}>COMUNIDADES CERCANAS</Text>
            <TouchableOpacity style={styles.groupCard}>
              <View style={[styles.dotIndicator, { backgroundColor: MODES[mode].color }]} />
              <View>
                <Text style={styles.groupName}>CENTRAL DE DATOS</Text>
                <Text style={styles.groupStats}>Transferencia de energía activa...</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  orbCore: { 
    width: ORB_SIZE - 50, height: ORB_SIZE - 50, borderRadius: (ORB_SIZE - 50) / 2, 
    justifyContent: 'center', alignItems: 'center', shadowRadius: 60, shadowOpacity: 1, borderWidth: 2 
  },
